...
#ifdef _MSC_VER
#define KAFKA_EXPORT    __declspec(dllexport)
#else
#define KAFKA_EXPORT    extern
#endif
#define DIST_VERSION "1.07"
...
static kafka_producer* check_producer(lua_State *lua,
                                      ...
                                      int max_args)
{
  kafka_producer *kp = (kafka_producer *)luaL_checkudata(lua, 1, mozsvc_kafka_producer);
...
static int producer_new(lua_State *lua)
{
  ...
  kafka_producer *kp = (kafka_producer *)lua_newuserdata(lua, sizeof(kafka_producer));
  kp->rk          = NULL;
  kp->msg_opaque  = NULL;
  kp->failures    = 0;
                              ...
                              const char *topic)
{
  ...
  if (lua_type(lua, -1) == LUA_TLIGHTUSERDATA) {
    kt = (kafka_topic *)lua_touserdata(lua, -1);;
  }
  lua_pop(lua, 2);
  return kt;
}
...
static int producer_create_topic(lua_State *lua)
{
  ...
  kafka_topic *kt = (kafka_topic *)malloc(sizeof(kafka_topic));
  if (!kt) {
...
static int producer_gc(lua_State *lua)
{
  ...
  while (lua_next(lua, -2) != 0) {
    kafka_topic *kt = (kafka_topic *)lua_touserdata(lua, -1);
    if (kt) {
      rd_kafka_topic_destroy(kt->rkt);
      free(kt);
    }
  ...
  // This may timeout because it might not be the last Lua running.
...
{
  kafka_consumer *kc = (kafka_consumer *)luaL_checkudata(lua, 1, mozsvc_kafka_consumer);
  int n = lua_gettop(lua);
                                ...
                                int cnt)
{
  ...
  while (lua_next(lua, 2) != 0) {
	...
	unsigned int topicsize = 0;
	const char *topic = lua_tolstring(lua, -1, &topicsize);
    const char *t;
    long partition = -1;

    if ((t = strstr(topic, ":"))) { // Parse "topic[:partition]
      char *s = (char *)malloc(topicsize);
...
static int consumer_new(lua_State *lua)
{
  ...
  kafka_consumer *kc = (kafka_consumer *)lua_newuserdata(lua, sizeof(kafka_consumer));
  kc->rk = NULL;
  kc->topics = NULL;
...
static int consumer_receive(lua_State *lua)
{
  ...
  if (rkmessage) {
    if (rkmessage->err) {
      ...
      lua_pushlstring(lua, (const char *)rkmessage->payload, rkmessage->len);
      lua_pushstring(lua, rd_kafka_topic_name(rkmessage->rkt));
      lua_pushinteger(lua, (lua_Integer)rkmessage->partition);
      if (rkmessage->key_len) {
        lua_pushlstring(lua, (const char *)rkmessage->key, rkmessage->key_len);
      } else {
        lua_pushnil(lua);
      }
    }
...
static struct luaL_reg consumerlib_m[] = {
...
KAFKA_EXPORT int luaopen_kafka(lua_State *lua)
{
